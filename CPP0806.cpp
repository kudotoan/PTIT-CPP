...
class HoaDon {
    ...
    public:
        ...
        friend ostream &operator<<(ostream &out, HoaDon a) {
            out << a.MHD << " " << KH[a.MKH].ten << " " << KH[a.MKH].diachi << " " << MH[a.MMH].tenMH << " " << MH[a.MMH].DVT << " " << MH[a.MMH].giamua << " " << MH[a.MMH].giaban << " " << a.sl << " " << MH[a.MMH].giaban*a.sl << endl;
            return out;
        }
};
int main(){
    ...
    MatHang dsmh[45];
    HoaDon dshd[105];
    ifstream KHinp("KH.in");
    int N,M,K,i;
    KHinp >> N;
    KHinp.ignore();
    for(i=0;i<N;i++) KHinp >> dskh[i];
    KHinp.close();
    ifstream MHinp("MH.in");
    MHinp >> M;
    MHinp.ignore();
    for(i=0;i<M;i++) MHinp >> dsmh[i];
    MHinp.close();
    ifstream HDinp("HD.in");
    HDinp >> K;
    HDinp.ignore();
    for(i=0;i<K;i++) HDinp >> dshd[i];
    HDinp.close();
